...
async function l({ api, message, event, args }) {
  try {
    ...
    if (args[0].toLowerCase() === "تخيل") {
      draw = true;
      n = args.slice(1).join(" ").trim();
    } else if (args[0].toLowerCase() === "ارسل") {
      sendTikTok = true;
      n = args.slice(1).join(" ").trim();
    } else if (args[0].toLowerCase() === "اغنية") {
      sing = true;
      n = args.slice(1).join(" ").trim();
    ...
    if (!n) {
      return message.reply("ادخل سؤالك.");
    }
...
const a = {
  ...
  countDown: 5,
  role: 0,
  longDescription: "التحدث مع بارد",
  category: "ai",
  guide: {
    en: "{p}بارد {سؤال}"
  }
